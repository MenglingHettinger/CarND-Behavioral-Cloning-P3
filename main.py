...
udacity_csv = load_csv(udacity_path + "/driving_log.csv", col_header_names)
...
gen_train = generate_images(ensemble_csv, (160, 320, 3), st_angle_names, "Steering Angle", st_angle_calibrations,  batch_size=128)

# Take 20% of the data for validation
print("Generate Validation Images...")
gen_val = generate_images(validation_csv, (160, 320, 3), st_angle_names, "Steering Angle", st_angle_calibrations,  batch_size=128, data_aug_pct=0.0)
x_val, y_val = next(gen_val)

# Train the model
m.fit_generator(gen_train, validation_data=(x_val, y_val), samples_per_epoch=len(ensemble_csv), nb_epoch=15, verbose=1)
